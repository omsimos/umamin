...
describe("NoteCard reactions", () => {
  it("reacts optimistically and calls the add action", async () => {
    ...
    const button = screen.getByRole("button", { name: /relate to this note/i });
    expect(button).toHaveAttribute("aria-pressed", "false");
    expect(button).toHaveTextContent("relate");
  ...
  it("removes a reaction and decrements the count", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /relate to this note/i }),
    ).toHaveTextContent("2");
  });
