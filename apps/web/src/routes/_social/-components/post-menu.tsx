...
export function PostMenu({
  ...
  const pinMutation = useMutation({
    ...
    onSuccess: (nowPinned) => {
      ...
      // the public posts route is CDN-cached, which a refetch would just read
      // back the pre-pin order from. Visitors converge on the CDN's own
      // schedule, same per-viewer-instant split as likes/reposts. The previous
      // pin only loses its badge here; its true chronological slot is restored
      // by the next natural revalidate.
