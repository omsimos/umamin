...
export function UserProfile({ username, initialUser }: Props) {
  ...
  // Client-side self-check (app-wide currentUser cache).
  ...
  const blockMutation = useMutation({
    ...
    onMutate: async (prevBlocked) => {
      ...
      // The inbox no longer carries sender ids (anonymity), so the blocked
      // sender's messages can't be filtered client-side — receivedMessages is
      // invalidated on success to refetch the server-filtered list instead.
