...
// The reaction/remove actions moved from apps/www's `@/app/actions/note` to the
// route group's colocated wrappers — mock that module here.
vi.mock("../-lib/actions", () => ({
...
// PostBody linkifies @mentions via the @/lib/navigation shim's <Link> (TanStack
// Router's Link, which needs a RouterProvider). Mock it to a plain <a> so the
// mention href assertion works standalone (matches quoted-post-card.test).
