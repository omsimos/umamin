...
type AuraTx = Parameters<Parameters<Db["transaction"]>[0]>[0];
 ...
 * beneficiary's username ONLY when the award actually lands (else null) — the
 * caller uses it to key its cache / query invalidation on the affected profile.
 * No-ops when:
