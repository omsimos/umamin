...
import { ReceivedMessageCard, messageFragment } from "./recent-message-card";
...
export async function RecentMessages({ userId }: { userId: string }) {
  const result = await getClient().query(MessagesQuery, {
    userId,
    type: "recent",
  });
  ...
  return (
    <div className="flex flex-col items-center gap-5 pb-20">
      {!messages?.length && (
        <p className="text-sm text-muted-foreground mt-4">No messages to show</p>
      )}
